  ...
  export default {
    ...
    methods: {
      async initialize() {
        ...
        const res = await entityApi.get().catch((error) => {
        ...
        this.records = res.data.entities;
        this.recordsFiltered = res.data.entities;
      },
      ...
      searchValue() {
        ...
        if (this.search != "") {
          this.records.forEach((record) => {
            let searchConcat = "";
            for (let i = 0; i < record.entity_name.length; i++) {
